...
double dist2_mm(Microcell* a, Microcell* b)
{
	...
	m = (int)(b - Mcells);
	if (P.DoUTM_coords)
		return dist2UTM(P.in_microcells_.width_ * fabs((double)(l / P.total_microcells_high_)), P.in_microcells_.height_ * fabs((double)(l % P.total_microcells_high_)),
			P.in_microcells_.width_ * fabs((double)(m / P.total_microcells_high_)), P.in_microcells_.height_ * fabs((double)(m % P.total_microcells_high_)));
	else
	{
		x = P.in_microcells_.width_ * fabs((double)(l / P.total_microcells_high_ - m / P.total_microcells_high_));
		y = P.in_microcells_.height_ * fabs((double)(l % P.total_microcells_high_ - m % P.total_microcells_high_));
		return periodic_xy(x, y);
	}
}
